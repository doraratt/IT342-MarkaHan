...
function Students() { // Removed `user` prop since we use useUser
  const { user } = useUser(); // Use context to get user
  ...
  const handleCloseAdd = () => {
    ...
    setNewStudent({ firstName: '', lastName: '', section: '', gradeLevel: '' });
  };
  ...
  const handleCloseEdit = () => {
    ...
    setEditingStudent({ id: null, firstName: '', lastName: '', section: '', gradeLevel: '' });
  };
  const handleOpenSearch = () => setOpenSearchModal(true);
  const handleCloseSearch = () => setOpenSearchModal(false);

  const handleAddStudent = async () => {
    if (!newStudent.firstName || !newStudent.lastName || !newStudent.section || !newStudent.gradeLevel) {
  ...
  const handleFinalEdit = async () => {
    if (!editingStudent.id || !editingStudent.firstName || !editingStudent.lastName || !editingStudent.section || !editingStudent.gradeLevel) {
